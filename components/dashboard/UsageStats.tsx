      ...
      <div className={`${className}`}>
        <Card className="p-6">
          <div className="text-center text-red-600">
            ...
            <button onClick={fetchUsageStats} className="mt-2 text-sm underline">
              Try again
            </button>
          </div>
        </Card>
            ...
            {recentActivity.slice(0, 5).map((activity, index) => (
              ...
              >
                <div className="flex items-center">
                  ...
                  <span className="text-sm text-gray-900">{formatActionName(activity.action)}</span>
                  {activity.metadata &&
                    typeof activity.metadata === 'object' &&
                    'service_name' in activity.metadata &&
                    typeof activity.metadata.service_name === 'string' && (
                      <span className="ml-2 text-xs text-gray-500">({activity.metadata.service_name})</span>
                    )}
                </div>
