...
export default function ShareStatusDashboard({ className = '' }: ShareStatusDashboardProps) {
  ...
  const getStatusBadge = (link: SharedLink) => {
    ...
    if (isAccessLimitReached(link)) {
      return (
        <span className="px-2 py-1 text-xs font-medium bg-orange-100 text-orange-800 rounded-full">Limit Reached</span>
      );
    }
      ...
      <div className={`${className}`}>
        <Card className="p-6">
          <div className="text-center text-red-600">
            ...
            <button onClick={fetchSharedLinks} className="mt-2 text-sm underline">
              Try again
            </button>
          </div>
        </Card>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                      <div className="flex items-center">
                        ...
                        <span>
                          {link.access_count}
                          {link.max_access_count && ` / ${link.max_access_count}`} access
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <CopyButton text={getShareUrl(link.token)} className="flex-shrink-0" />
                      <Button
                        variant="secondary"
                        size="sm"
