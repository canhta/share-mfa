...
export default function ShareStatusDashboard({
  className = "",
}: ShareStatusDashboardProps) {
  ...
  const fetchSharedLinks = async () => {
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load shared links",
      );
    } finally {
      setIsLoading(false);
    }
        ...
        {sharedLinks.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-sm text-gray-400">
              Create your first MFA entry and generate a sharing link to get
              started
            </p>
          </div>
        ) : (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center mb-2">
                      <h4 className="font-medium text-gray-900 mr-3">
                        {link.service_name ||
                          `MFA Entry ${link.mfa_entry_id.slice(-8)}`}
                      </h4>
                      {getStatusBadge(link)}
                    </div>
