...
export default function OnboardingLayout({ children, currentStep, totalSteps = 3 }: OnboardingLayoutProps) {
  return (
            ...
            <ol className="flex items-center justify-center space-x-4 sm:space-x-8">
              {steps.map((step) => (
                <li key={step.id} className="flex items-center">
                  ...
                  {step.id < totalSteps && (
                    <div className={`ml-4 h-px w-8 sm:w-16 ${step.id < currentStep ? 'bg-blue-600' : 'bg-gray-300'}`} />
                  )}
                </li>
              ))}
            </ol>
