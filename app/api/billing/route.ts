...
export async function GET() {
  try {
    ...
    const { data: profile, error: profileError } = await supabase
      .from("profiles")
      .select(
        "subscription_status, user_tier, available_credits, current_period_end",
      )
      .eq("id", user.id)
      .single();
    ...
    const now = new Date();
    const trialEndsAt = profile.current_period_end
    ...
    const currentPlan =
      planDetails[profile.user_tier as keyof typeof planDetails] ||
      planDetails.free;
