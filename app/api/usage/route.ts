...
export async function GET() {
  try {
    ...
    const weeklyEvents = usageEvents.filter(
      (event) => new Date(event.created_at) >= sevenDaysAgo,
    );

    const aggregated = {
      total: {
        shares_generated: usageEvents.filter(
          (e) => e.action === "share_generated",
        ).length,
        shares_accessed: usageEvents.filter(
          (e) => e.action === "share_accessed",
        ).length,
        mfa_entries_added: usageEvents.filter((e) => e.action === "mfa_added")
          .length,
        qr_codes_scanned: usageEvents.filter((e) => e.action === "qr_scanned")
          .length,
      },
      last_30_days: {
        shares_generated: recentEvents.filter(
          (e) => e.action === "share_generated",
        ).length,
        shares_accessed: recentEvents.filter(
          (e) => e.action === "share_accessed",
        ).length,
        mfa_entries_added: recentEvents.filter((e) => e.action === "mfa_added")
          .length,
        qr_codes_scanned: recentEvents.filter((e) => e.action === "qr_scanned")
          .length,
      },
      last_7_days: {
        shares_generated: weeklyEvents.filter(
          (e) => e.action === "share_generated",
        ).length,
        shares_accessed: weeklyEvents.filter(
          (e) => e.action === "share_accessed",
        ).length,
        mfa_entries_added: weeklyEvents.filter((e) => e.action === "mfa_added")
          .length,
        qr_codes_scanned: weeklyEvents.filter((e) => e.action === "qr_scanned")
          .length,
      },
    };
