...
export async function GET(request: NextRequest) {
  try {
    ...
    const tierDistribution = {
      ...
      enterprise:
        userStats?.filter((u) => u.user_tier === "enterprise").length || 0,
    };

    // Process subscription status distribution
    const subscriptionDistribution = {
      active:
        userStats?.filter((u) => u.subscription_status === "active").length ||
        0,
      canceled:
        userStats?.filter((u) => u.subscription_status === "canceled").length ||
        0,
      past_due:
        userStats?.filter((u) => u.subscription_status === "past_due").length ||
        0,
      trialing:
        userStats?.filter((u) => u.subscription_status === "trialing").length ||
        0,
    ...
    const leadStats = {
      ...
      byTierInterest: {
        ...
        enterprise:
          leadsData?.filter((l) => l.tier_interest === "enterprise").length ||
          0,
        newsletter:
          leadsData?.filter((l) => l.tier_interest === "newsletter").length ||
          0,
      },
      bySource:
    ...
    const revenueStats = {
      totalRevenue:
        revenueData?.reduce((sum, event) => sum + (event.amount || 0), 0) || 0,
      subscriptions:
        revenueData?.filter((e) => e.event_type === "subscription").length || 0,
      upgrades:
        revenueData?.filter((e) => e.event_type === "upgrade").length || 0,
      downgrades:
        revenueData?.filter((e) => e.event_type === "downgrade").length || 0,
